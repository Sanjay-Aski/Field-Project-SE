...
const AttendanceEntryPage = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.103.107:5000/teacher/class-students?class=${selectedClass}&division=${selectedDivision}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    try {
      ...
      const attendancePromises = Object.entries(attendance).map(async ([studentId, isPresent]) => {
        if (isPresent) {
          ...
          const response = await fetch('http://192.168.103.107:5000/teacher/assign-attendance', {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      if (activeTab === 'upload-attendance') {
        endpoint = 'http://192.168.103.107:5000/teacher/assign-attendance-excel';
      } else if (activeTab === 'working-days') {
        endpoint = 'http://192.168.103.107:5000/teacher/set-working-days-excel';
      }
  ...
  const handleSetWorkingDays = async () => {
    try {
      ...
      const response = await fetch('http://192.168.103.107:5000/teacher/set-working-days', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
